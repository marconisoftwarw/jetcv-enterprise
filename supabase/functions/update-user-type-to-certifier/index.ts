...
Deno.serve(async (req: Request) => {
  ...
  console.log(`[${reqId}] Starting update-user-type-to-certifier function`);
  ...
  try {
    ...
    // Parse request body
    let requestData: UpdateUserTypeRequest;
    try {
    ...
    const { data: updatedUser, error: updateError } = await admin
      ...
      .eq('idUser', requestData.user_id)
      .select('idUser, firstName, lastName, email, type, updatedAt')
      .single();
    ...
    console.log(`[${reqId}] Successfully updated user type to certifier:`, {
    ...
    if (requestData.legal_entity_id) {
      ...
      const { data: certifier, error: certifierError } = await admin
        .from('certifier')
        .insert({
        ...
        .select('id_certifier, id_certifier_hash, id_legal_entity, id_user, active, role')
        .single();
    ...
    const response = {
      success: true,
      message: 'User type updated to certifier successfully',
      data: {
        user: updatedUser,
