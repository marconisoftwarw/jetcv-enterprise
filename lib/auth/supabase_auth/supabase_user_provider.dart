...
class JetCVEnterpriseSupabaseUser extends BaseAuthUser {
  JetCVEnterpriseSupabaseUser(this.user);
  User? user;
...
Stream<BaseAuthUser> jetCVEnterpriseSupabaseUserStream() {
  ...
  return (!loggedIn
      ...
      .map<BaseAuthUser>(
    (authState) {
      currentUser = JetCVEnterpriseSupabaseUser(authState?.session?.user);
      return currentUser!;
    },
  );
}
