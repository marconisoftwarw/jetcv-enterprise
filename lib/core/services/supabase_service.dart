import 'package:supabase_flutter/supabase_flutter.dart';
import 'package:gotrue/src/types/user.dart' as gotrue;
import '../../shared/models/user_model.dart';
import '../../shared/models/legal_entity_model.dart';
import '../config/app_config.dart';
import '../constants/app_constants.dart';

class SupabaseService {
  static final SupabaseService instance = SupabaseService._();
  
  SupabaseService._();
  
  SupabaseClient? _client;
  bool _isInitialized = false;
  
  SupabaseClient? get client => _client;
  bool get isInitialized => _isInitialized;
  
  Future<void> initialize() async {
    try {
      // Check if credentials are properly configured
      if (AppConfig.supabaseAnonKey == 'your-anon-key') {
        print('⚠️ Supabase credentials not configured. Running in demo mode.');
        _isInitialized = false;
        return;
      }
      
      await Supabase.initialize(
        url: AppConfig.supabaseUrl,
        anonKey: AppConfig.supabaseAnonKey,
      );
      _client = Supabase.instance.client;
      _isInitialized = true;
      print('✅ Supabase initialized successfully');
    } catch (e) {
      print('❌ Error initializing Supabase: $e');
      _isInitialized = false;
    }
  }
  
  // Authentication methods
  Future<gotrue.AuthResponse> signUpWithEmail({
    required String email,
    required String password,
    required Map<String, dynamic> userData,
  }) async {
    if (!_isInitialized) {
      throw Exception('Supabase non è configurato. Configura le credenziali per utilizzare l\'app.');
    }
    
    return await _client!.auth.signUp(
      email: email,
      password: password,
      data: userData,
    );
  }
  
  Future<gotrue.AuthResponse> signInWithEmail({
    required String email,
    required String password,
  }) async {
    if (!_isInitialized) {
      throw Exception('Supabase non è configurato. Configura le credenziali per utilizzare l\'app.');
    }
    
    return await _client!.auth.signInWithPassword(
      email: email,
      password: password,
    );
  }
  
  Future<void> signOut() async {
    if (!_isInitialized) return;
    await _client!.auth.signOut();
  }
  
  gotrue.User? get currentUser {
    if (!_isInitialized) return null;
    return _client!.auth.currentUser;
  }
  
  Stream<gotrue.User?> get authStateChanges {
    if (!_isInitialized) return Stream.value(null);
    return _client!.auth.onAuthStateChange.map((event) => event.session?.user);
  }
  
  // User data methods
  Future<UserModel?> getUserById(String userId) async {
    if (!_isInitialized) return null;
    
    try {
      final response = await _client!.from('user').select().eq('idUser', userId).single();
      return UserModel.fromJson(response);
    } catch (e) {
      print('Error getting user by ID: $e');
      return null;
    }
  }
  
  Future<UserModel?> getCurrentUserData() async {
    if (!_isInitialized) return null;
    
    final user = currentUser;
    if (user == null) return null;
    
    return await getUserById(user.id);
  }
  
  Future<void> updateUserData(String userId, Map<String, dynamic> data) async {
    if (!_isInitialized) return;
    
    try {
      await _client!.from('user').update(data).eq('idUser', userId);
    } catch (e) {
      print('Error updating user data: $e');
      rethrow;
    }
  }
  
  // Legal entity methods
  Future<List<LegalEntity>> getLegalEntities() async {
    if (!_isInitialized) return [];
    
    try {
      final response = await _client!.from('legal_entity').select().order('createdAt', ascending: false);
      return response.map((json) => LegalEntity.fromJson(json)).toList();
    } catch (e) {
      print('Error getting legal entities: $e');
      return [];
    }
  }
  
  Future<LegalEntity?> getLegalEntityById(String id) async {
    if (!_isInitialized) return null;
    
    try {
      final response = await _client!.from('legal_entity').select().eq('idLegalEntity', id).single();
      return LegalEntity.fromJson(response);
    } catch (e) {
      print('Error getting legal entity by ID: $e');
      return null;
    }
  }
  
  Future<void> createLegalEntity(Map<String, dynamic> data) async {
    if (!_isInitialized) return;
    
    try {
      await _client!.from('legal_entity').insert(data);
    } catch (e) {
      print('Error creating legal entity: $e');
      rethrow;
    }
  }
  
  Future<void> updateLegalEntity(String id, Map<String, dynamic> data) async {
    if (!_isInitialized) return;
    
    try {
      await _client!.from('legal_entity').update(data).eq('idLegalEntity', id);
    } catch (e) {
      print('Error updating legal entity: $e');
      rethrow;
    }
  }
  
  Future<void> updateLegalEntityStatus(String id, String status, String? reason, String adminUserId) async {
    if (!_isInitialized) return;
    
    try {
      final updateData = {
        'status': status,
        'statusUpdatedAt': DateTime.now().toIso8601String(),
        'statusUpdatedByIdUser': adminUserId,
      };
      
      if (reason != null) {
        updateData['rejectionReason'] = reason;
      }
      
      await _client!.from('legal_entity').update(updateData).eq('idLegalEntity', id);
    } catch (e) {
      print('Error updating legal entity status: $e');
      rethrow;
    }
  }
  
  // Admin methods
  Future<List<UserModel>> getUsers() async {
    if (!_isInitialized) return [];
    
    try {
      final response = await _client!.from('user').select().order('createdAt', ascending: false);
      return response.map((json) => UserModel.fromJson(json)).toList();
    } catch (e) {
      print('Error getting users: $e');
      return [];
    }
  }
  
  // Edge Functions
  Future<Map<String, dynamic>> callEdgeFunction(String functionName, Map<String, dynamic> data) async {
    if (!_isInitialized) {
      throw Exception('Supabase non è configurato. Configura le credenziali per utilizzare l\'app.');
    }
    
    try {
      final response = await _client!.functions.invoke(functionName, body: data);
      return response.data;
    } catch (e) {
      print('Error calling edge function $functionName: $e');
      rethrow;
    }
  }
  
  Future<void> sendLegalEntityInvitation(String email, String legalEntityName, String invitationLink) async {
    if (!_isInitialized) return;
    
    try {
      await callEdgeFunction('send-legal-entity-invitation', {
        'email': email,
        'legalEntityName': legalEntityName,
        'invitationLink': invitationLink,
      });
    } catch (e) {
      print('Error sending legal entity invitation: $e');
      rethrow;
    }
  }
}

// Provider for SupabaseService
final supabaseServiceProvider = Provider<SupabaseService>((ref) {
  return SupabaseService.instance;
});

// Provider for SupabaseClient
final supabaseClientProvider = Provider<SupabaseClient?>((ref) {
  return ref.watch(supabaseServiceProvider).client;
});
